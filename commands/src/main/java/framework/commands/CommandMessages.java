...
import static framework.commands.style.CommandStyleConfig.systemSecondaryColor;
import static net.kyori.adventure.text.Component.text;
import static net.kyori.adventure.text.Component.translatable;

import framework.commands.schema.Argument;
import framework.commands.schema.ArgumentSchema;
import framework.commands.schema.ArgumentSchemaHolder;
import framework.commands.scope.AbstractScopedParentCommand;
import framework.commands.style.CommandStyleConfig;
import framework.commons.Exceptions;
import framework.locale.MessageMapper;
...
public final class CommandMessages {
    ...
    public static final MessageMapper PLACEHOLDER_COMMAND_FORMAT = args -> {
      ...
      if (command instanceof ArgumentSchemaHolder) {
        ...
        if (command instanceof AbstractScopedParentCommand) { // if our command is scoped
          appendix
              .append(((AbstractScopedParentCommand<?>) command).specificAppendix()); // return appendix
        }

        if (!command.childCommands().isEmpty()) {
          appendix.append(PLACEHOLDER_COMMAND_FORMAT_HAS_SUB_ARGS); // if command have children, append suffix that it's not end
        }
      }
  ...
  public static final class Error {
    ...
    public static final MessageMapper PARSER_PATTERN = args ->
        error().append(key("parser.pattern").args(remapString(0, args), remapString(1, args))); // arg0 = input, arg1 = pattern

    public static final MessageMapper PARSER_STRING = args ->
        error().append(key("parser.string")
            .args(remapString(0, args), remapString(1, args))); // arg0 = input, arg1 = all allowed strings
            ...
            .args(
                remapString(0, args)
                    ...
                    .hoverEvent(HoverEvent.showText(
                        remapString(1, args).color(CommandStyleConfig.systemPrimaryColor()))) // arg0 = arg name
            )
            .asComponent();
